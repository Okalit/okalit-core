...
async function executeLoader(host) {
  ...
  try {
    ...
    host.dispatchEvent(new CustomEvent('o-error', {
      detail: err,
      bubbles: true,
      composed: true
    }));
...
export class OViewport extends HTMLElement {
  ...
  constructor() {
    ...
    style.textContent = `
      ...
      :host(:not([loaded])) ::slotted([slot="fallback"]) { display: contents; }
    `;
    this.shadowRoot.append(style, this.#sentinel, document.createElement('slot'), Object.assign(document.createElement('slot'), {
      name: 'fallback'
    }));
  }
  connectedCallback() {
